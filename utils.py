...
from torch_geometric.utils import add_remaining_self_loops, subgraph, degree
...
def construct_knn(kernel_idx, jug):
    edge_index = [[], []]
...
def get_kernel_knn(dataname, kernel_type, knn_nei_num, jug):
    kernel_file = './kernel/' + dataname + '_' + \
        kernel_type + '.txt'
    if dataname == 'REDDITBINARY':
    ...
    knn_edge_index = construct_knn(kernel_idx, jug)
